...
export function GameLibrary({ games }: { games: readonly GameCatalogEntry[] }) {
  ...
  return (
    <>
      <div className={styles.libraryControls} role="search" aria-label="Filter game library">
        ...
        <label className={styles.searchField}>
          <span className="visually-hidden">Search games, genres, or features</span>
          <input aria-controls="game-library-results" aria-describedby="game-library-result-count" aria-keyshortcuts="Escape" autoCapitalize="none" autoComplete="off" enterKeyHint="search" inputMode="search" spellCheck={false} value={query} onChange={(event) => setQuery(event.target.value)} onKeyDown={(event) => { if (event.key === "Escape" && query) { event.preventDefault(); setQuery(""); } }} placeholder="Search games, genres, or features" type="search" />
        </label>
      </div>

      <p className={styles.resultCount} id="game-library-result-count" aria-live="polite" aria-atomic="true">
        Showing {visibleGames.length} of {games.length} total {games.length === 1 ? "game" : "games"}
      </p>
      ...
      {visibleGames.length > 0 ? (
        ...
        <div className={styles.emptyState} id="game-library-results" role="region" aria-label="Game library results">
          ...
          <button type="button" aria-controls="game-library-results" onClick={() => { setFilter("all"); setQuery(""); }}>Clear filters and show every game</button>
        </div>
      )}
    </>
  );
