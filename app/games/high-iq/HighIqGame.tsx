...
function calculateDailyStreak(dates: string[]) {
...
export default function HighIqGame() {
  ...
  useEffect(() => {
    try {
      ...
      if (Array.isArray(storedMissed)) setMissedIds(storedMissed.filter((value) => typeof value === "string"));

      const storedDates = JSON.parse(window.localStorage.getItem(DAILY_DATES_KEY) ?? "[]") as string[];
      if (Array.isArray(storedDates)) setDailyDates(storedDates.filter((value) => typeof value === "string"));
    } catch {
  ...
  useEffect(() => {
    ...
    const missed = answerRecords.filter((record) => !record.correct).map((record) => record.id);
    ...
    setResultSaved(true);
  }, [answerRecords, correctCount, dailyDates, deck.length, phase, resultSaved, runMode, score, today]);
      ...
      <div id={GAME_ID} className={styles.gameFrame}>
        <section className={styles.resultsStage}>
          ...
          <div className={styles.resultActions}>
            <button className={styles.dailyButton} type="button" onClick={runMode === "daily" ? startDaily : startCustom}>
              Play again
            </button>
            {missedIds.length ? (
