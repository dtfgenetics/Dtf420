...
function findModule(slug: string) {
  return modules.find((module) => slugFor(module.id) === slug);
}

export function generateStaticParams() {
  return modules.map((module) => ({ system: slugFor(module.id) }));
}

export async function generateMetadata({ params }: { params: Promise<{ system: string }> }): Promise<Metadata> {
  const { system } = await params;
  const module = findModule(system);
  if (!module) return { title: "Atlas System" };
  const section = atlasSections.find((item) => item.id === module.id);
  return {
    title: `${module.label} — THC Living Plant Atlas`,
    description: section?.summary ?? `Visual cannabis plant science lessons for ${module.label}.`,
  };
}
...
export default async function AtlasSystemPage({ params }: { params: Promise<{ system: string }> }) {
  ...
  const section = atlasSections.find((item) => item.id === module.id);
  const related = modules.filter((item) => item.id !== module.id).slice(0, 4);
      ...
      <div className={styles.pageShell}>
        ...
        <section className={styles.hero}>
          <div className={styles.heroCopy}>
            ...
            <div className={styles.stats}>
              <span><b>{module.lessons.length}</b> structured lessons</span>
              <span><b>{module.learningGoals.length}</b> learning goals</span>
              <span><b>{section?.topics.length ?? 0}</b> indexed topics</span>
            </div>
          </div>
          <AtlasSystemGraphic systemId={module.id} />
        </section>
        ...
        <section className={styles.lessonsSection}>
          <header>
            ...
            <h2>{module.label} lesson sequence</h2>
