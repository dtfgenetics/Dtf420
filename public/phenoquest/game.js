...
export async function startPhenoQuest(THREE) {
  ...
  function animate(now) {
    if (disposed) return;
    const frameDelta = Math.min(MAX_FRAME_DELTA, Math.max(0, (now - lastTime) / 1000));
    lastTime = now;
    ...
    const visualDt = frameDelta || PHYSICS_STEP;
    updateCamera(visualDt);

    starterGlow.rotation.z += visualDt * 0.55;
    trialRing.rotation.z -= visualDt * 0.32;
    for (const [id, marker] of encounterMarkers) {
      if (!marker.visible) continue;
      marker.rotation.y += visualDt * (id.length % 2 ? 0.7 : -0.6);
