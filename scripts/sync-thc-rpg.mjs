...
async function fetchPinnedFile(relativePath, expectedSha) {
  ...
  if (!response.ok) {
    throw new Error(`THC RPG sync failed for ${relativePath}: HTTP ${response.status}`);
  }
...
console.log(
  `THC RPG runtime ready from ${SOURCE_REPOSITORY}@${SOURCE_COMMIT} (${synced} synced, ${cached} cached, ${Object.keys(manifest).length} verified).`,
);
