...
const [route, html, css, runtime, dataText] = await Promise.all(
...
requireText(runtime, "const PHYSICS_STEP = 1 / 60", "fixed physics step");
requireText(runtime, "const MAX_FRAME_DELTA = 0.5", "low-FPS frame catch-up cap");
requireText(runtime, "while (remaining > 0)", "low-FPS physics substeps");
...
console.log(`PhenoQuest core verified: ${data.phenos.length} Phenos, ${data.starters.length} starters, ${data.encounters.length} field encounters, Team Lockout threshold ${data.progression.samplesBeforeLockout}, full-width gate, battle turn lock, Garden Trial boss ${data.progression.gardenTrialBoss}.`);
